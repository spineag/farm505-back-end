            ...
            $shardDb = $app->getShardDb($userId, $channelId);
                ...
                if ($channelId == 2) { // VK
                    ...
                    if ($channelId == 3) { // OK
                        ...
                        try {
                            $result = $mainDb->query('SELECT * FROM trabsaction_webhooks WHERE user_social_id="'.$socialId.'"');
                            $ar = $result->fetchAll();
                            if ($ar && count($ar)) {
                                foreach ($ar as $key => $rqst) {
                                    $requestId = $rqst['request_id'];
                                    $result = $mainDb->query('SELECT * FROM transactions WHERE request_id="'.$requestId.'" AND status="start"');
                                    $trans = $result->fetch();
                                    if ($trans) {
                                        if ($trans['product_code'] == '13') {
                                            $result = $mainDb->query('UPDATE users SET starter_pack = 1 WHERE id=' . $_POST['userId']);
                                            $result = $mainDb->query("SELECT * FROM data_starter_pack");
                                            $startPackData = $result->fetch();
                                            $oType = (int)$startPackData['object_type'];
                                        ...
                                        $result = $mainDb->query('UPDATE transactions SET status="complete_webhooks" WHERE request_id="'.$requestId.'"');
                                    }
                                }
                                $result = $mainDb->query('DELETE FROM trabsaction_webhooks WHERE user_social_id="'.$socialId.'"');
                            }
                        } catch (Exception $e) {  }
                    }
                }
