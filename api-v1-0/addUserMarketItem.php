...
if (isset($_POST['userId']) && !empty($_POST['userId'])) {
    ...
    if ($app->checkSessionKey($_POST['userId'], $_POST['sessionKey'])) {
        ...
        if ($m != $_POST['hash']) {
            ...
            try {
                $time = time();
                $result = $mainDb->queryWithAnswerId('INSERT INTO user_market_item SET user_id='.$_POST['userId'].', buyer_id=0, resource_id='.$_POST['resourceId'].', time_start='.$time .',time_sold=0, cost='.$_POST['cost'].', resource_count='.$_POST['count'].', in_papper='.$_POST['inPapper'].', number_cell='.$_POST['numberCell'].', time_in_papper='.$_POST['timeInPapper'].', level='.$_POST['level']);
                if ($result) {
                    $res = [];
                    $res['id'] = $result[1];
                    $res['buyer_id'] = 0;
