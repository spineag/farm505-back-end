        ...
        try {
            ...
            if ($channelId == 2) {
                ...
                foreach ($arShards as $key => $shard) {
                    $result = $shard->query("SELECT * FROM user_market_item WHERE in_papper = 1 AND buyer_id = 0 AND time_in_papper > ".$endTime." AND level <= ".$_POST['level']." AND user_id <> ".$userId." ORDER BY RAND() LIMIT 60");
                    $arr = $result->fetchAll();
