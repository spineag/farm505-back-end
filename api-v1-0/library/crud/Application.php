...
require_once __DIR__ . "/../../config/configNew.php";
require_once __DIR__ . "/../../tools/SocialNetwork.php";
...
class Application
{
    ...
    function __construct()
    {
        ...
        $cfgs = $GLOBALS["cfgs"];
        $serverName = "i-vk-birds-island.joyrocks.com";
...
//        }
    ...
    final public function newUser($channelId, $socialUId, $sex = 'u', $bDate = '')
    {
        ...
        $result = $mainDb->insert(
            $tableName,
            ['user_social_id' => $socialUId, 'created_date' => time(), 'last_visit_date' => time(), 'sex' => $sex, 'birthdate' => $bDate, 'channel_id' => $channelId, 'tutorial_step' => 1 ],
            ['int', 'int', 'int', 'str', 'str', 'int', 'int']);
    ...
    {
        if ($channelId > 0 && $userSocialId > 0)
        {
            $mainDb = $this->getMainDb();
            $result = $mainDb->query("SELECT u.user_id, u.volume, u.music, u.payer, u.tutorial_step, u.is_daily_bonus, count(t.user_id) AS tester, t.megatester AS megatester
                                      FROM users u
